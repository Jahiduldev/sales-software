...
class General_request extends CI_Controller {
    ...
    public function index() {
        ...
        if (in_array($this->session->userdata('role_id'), $per_role_arr)):
            ...
            $data['active_sub_menu'] = 'General NSP';
			$data['getNsp']= $this->common_model->getNsp();

            $this->load->view('common/header',$data);
            $this->load->view('general_request/general_request',$data);
            $this->load->view('common/js',$data);
            $this->load->view('common/footer',$data);
            $this->load->view('general_request/js_general_request',$data);
    ...
    public function addRequestService() {
        ...
        if (in_array($this->session->userdata('role_id'), $per_role_arr)):
            ...
            $detail_id= mysql_real_escape_string($this->input->post('detail_id'));
            ...
            $update_result = $this->common_model->updateData($table_name,$data2,'de_id',$detail_id);
    ...
    public function getTableData() {
        ...
        $columns = array(
				...
				array('db' => '`d`.`de_id`', 'dt' => 7, 'field' =>'de_id', 'formatter' => function ($rowvalue, $row){
                            return '<a  href="#">
		...
		JOIN `base_service_details` AS `d` ON (`m`.`m_id` = `d`.`ref_master_id` AND `d`.`service_type`=2) 
		...
		JOIN `base_service_custom` AS `cu` ON (`cu`.`ref_client_id` = `c`.`ci_id` AND `cu`.`service_status`=2)
        group by `d`.`ref_client_id` HAVING DATEDIFF('$today', max(`d`.`service_date`))>120";

       // $extraCondition="";
       // $groupBy="`d`.`ref_client_id`";
       // $havingCondition="DATEDIFF('$today',max(`d`.`service_date`))>120";
    ...
    public function sendSms() {
        ...
        $joinQuery = "SELECT `c`.`ci_id`,`c`.`client_phone`,`d`.`de_id`   FROM `base_service_master` AS `m`  
		JOIN `base_service_details` AS `d` ON (`m`.`m_id` = `d`.`ref_master_id`)
        JOIN `base_clients` AS `c` ON (`c`.`ci_id` = `m`.`ref_client_id`)  
		JOIN `nsp_status` AS `ns` ON (`m`.`is_in_service` = `ns`.`id`)
        JOIN `base_areas` AS `a` ON (`a`.`a_id` = `c`.`ref_area_id`)
...
$counter=0;
        if($result->num_rows()>0):
            ...
            foreach($result->result() as $row):
                ...
                $table_name='base_service_details';
                $update_result = $this->common_model->updateData($table_name,$data2,'de_id',$de_id);
    ...
    public function getData() {
	 ...
	 public function getDateSearch() { 
		...
		$getTable = $this->common_model->getNspDateWise($fmb4, $tob4);
		echo  $getTable;
        ...
        if (in_array($this->session->userdata('role_id'), $per_role_arr)){
			...
			if($clientresponse==1){
				...
				$data = array(
					...
					'service_description' => 'From Genaral NSP',
					'service_status' => 0,
					'is_active' => 1,
					'service_priority' => 5,
					'note_text' => $add_notes,
