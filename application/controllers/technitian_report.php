...
class Technitian_report extends CI_Controller {
    ...
    public function index() {
        ...
        if (in_array($this->session->userdata('role_id'), $per_role_arr)):
            ...
            $data['active_sub_menu'] = 'Technitian Assign Report';
            ...
            $data['technitian'] = $this->common_model->getTechnitian();
            $data['getTable'] = '';
            $data['dashboard'] = '';
            ...
            $this->load->view('technitian_report/technitian_report', $data);
            $this->load->view('common/footer', $data);
            $this->load->view('common/js', $data);
            $this->load->view('technitian_report/js_technitian_report', $data);
     ...
     public function dateSearch() {
        ...
        if (in_array($this->session->userdata('role_id'), $per_role_arr)):
            ...
            $column_name = 'service_status';
            $column_value = 0;
            $result=$this->db->query("select * from base_service_custom where service_status=0 and 
			(`request_date` between '$date_from' and '$date_to') ");
	 ...
	 public function getDateSearch() { 
		...
		$type = mysql_real_escape_string($this->input->post('proccess_type')); 
		$tech = mysql_real_escape_string($this->input->post('technitian')); 
		$dashboard = mysql_real_escape_string($this->input->post('dashboard')); 
		...
		$data['technitian'] = $this->common_model->getTechnitian($tech);
		...
		if(($fm != '' && $to != '')||($fm == '' && $to == '')){
			...
			$data['getTable'] = $this->common_model->getTechDateWise($fm, $to, $type, $tech);		
			$this->load->view('common/header', $data);
			$this->load->view('technitian_report/technitian_report', $data);
			$this->load->view('common/footer', $data);
			$this->load->view('common/js', $data);
