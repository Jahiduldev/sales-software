...
class General_request_filter extends CI_Controller {
    ...
    public function index() {
        ...
        if (in_array($this->session->userdata('role_id'), $per_role_arr)):
            ...
            $data['active_sub_menu'] = 'General Filter';
			$data['getFilter']= $this->common_model->getFilter();

            $this->load->view('common/header',$data);
            $this->load->view('general_request_filter/general_request_filter',$data);
            $this->load->view('common/js',$data);			
            $this->load->view('general_request_filter/js_general_request_filter',$data);
    ...
    public function addRequestService() {
        ...
        if (in_array($this->session->userdata('role_id'), $per_role_arr)):
            ...
            $filter_id= mysql_real_escape_string($this->input->post('filter_id'));
            ...
            $update_result = $this->common_model->updateData($table_name,$data2,'fil_id',$filter_id);
            ...
            redirect('general_request_filter');
        else:
    ...
    public function sendSms() {
        ...
        $joinQuery = "SELECT `c`.`ci_id`,`c`.`client_phone`,`fil`.`fil_id`   FROM `base_clients` AS `c`  JOIN `base_filter_changes` AS `fil` ON (`c`.`ci_id` = `fil`.`ref_client_id`)
...
$counter=0;
        if($result->num_rows()>0):
            ...
            foreach($result->result() as $row):
                ...
                $table_name='base_filter_changes';
                $update_result = $this->common_model->updateData($table_name,$data2,'fil_id',$fil_id);

                $data3 = array(

                        'ref_client_id' => $ci_id,
                        'request_date' => date('Y-m-d'),
                        'service_description' => "filter change via sms",
                        'service_status'=>0,
                        'is_active' => 1,
    ...
    public function getTableDataFilter() {
        ...
        $columns = array(
                ...
                array('db' => '`c`.`client_phone`', 'dt' => 2,'field' =>'client_phone'),
                array('db' => '`c`.`client_address`', 'dt' => 3,'field' =>'client_address'),
                array('db' => '`fil`.`fil_id`', 'dt' => 4,'field' =>'fil_id','formatter' => function ($rowvalue, $row) {
                            return '<a  href="#" >
      <button class="btn btn-primary btn-xs" onclick="addModal('.$rowvalue.')">Add</button></a>';
		...
		JOIN `base_filter_changes` AS `fil` ON (`c`.`ci_id` = `fil`.`ref_client_id`)
        group by `fil`.`ref_client_id` HAVING DATEDIFF('$today',max(`fil`.`change_date`))>365";

         //  $extraCondition="";
         //  $groupBy="`fil`.`ref_client_id`";
         //  $havingCondition="DATEDIFF('$today',max(`fil`.`change_date`))>365";


        echo json_encode(
       // SSP::simple($_GET, $sql_details, $table, $primaryKey, $columns,$joinQuery,$extraCondition,$groupBy,$havingCondition)
	 ...
	 public function getDateSearch() { 
		...
		$fmb4 = strtotime('-1 year', strtotime($fm));
		$fmb4 = date('Y-m-d', $fmb4);
		$tob4 = strtotime('-1 year', strtotime($to));
		$tob4 = date('Y-m-d', $tob4);

		$getTable = $this->common_model->getFilterDateWise($fmb4, $tob4);
		echo  $getTable;
    ...
    public function getData() {
        ...
        $ci_id = mysql_real_escape_string($this->input->post('d_id'));
		...
		$this->db->where('base_clients.ci_id', $ci_id);	        
		$result = $this->db->get();
        ...
        if (in_array($this->session->userdata('role_id'), $per_role_arr)){
			...
			if($clientresponse==1){
				...
				$data = array(
					...
					'service_description' => 'From Genaral Filter',
					'service_status' => 0,
					'is_active' => 1,
					'service_priority' => 5,
					'note_text' => $add_notes,
			...
			$user_id = $this->session->userdata('user_id');
				 ...
				 $data49 = array(
                    'uid' => $user_id,
					'action_name' => "General Request Filter",
