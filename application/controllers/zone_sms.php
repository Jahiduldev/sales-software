...
class Zone_sms extends CI_Controller {
    ...
    public function index() {
        ...
        if (in_array($this->session->userdata('role_id'), $per_role_arr)):
            ...
            $data['active_sub_menu'] = 'Zone Wise SMS';
            ...
            $this->load->view('zonesms/zonesms', $data);
            $this->load->view('common/footer', $data);
            $this->load->view('common/js', $data);
            $this->load->view('zonesms/js_zonesms', $data);
	...
	public function getDateSearch() { 
		...
		$data['active_menu'] = 'Reports';
        $data['active_sub_menu'] = 'Zone Wise SMS';		
		$tech = $this->input->post('technitian');	
        $smstext= $this->input->post('smstext');	
		$data['technitian'] = $this->common_model->getzones($tech);	            
		
		$complaintext = urlencode($smstext);
		$url2=''; $data['getTable']='';

		$this->db->where('ref_area_id', $tech);			
		$query = $this->db->get('base_clients'); $numbers = $query->num_rows();
		foreach($query->result() as $row){ 
		
			$sms_slice = explode('/',$row->client_phone);
			$msist = $sms_slice[0];
			$msis = strlen($msist);			
			if($msis==11){
				$msisfinal = '88'.$msist;			
...
//$data['getTable'] .= $url2.'<br>';		
		...
		$data['getTable'] = $numbers.' SMS Send';		
		$this->load->view('common/header', $data);
		$this->load->view('zonesms/zonesms', $data);
		$this->load->view('common/footer', $data);
		$this->load->view('common/js', $data);
