...
class Add_User extends CI_Controller {
    ...
    public function editUser() {
        ...
        if (in_array($this->session->userdata('role_id'), $per_role_arr)):
            ...
            $edit_role = mysql_real_escape_string($this->input->post('edit_role'));
    ...
    public function changeUser() {
        ...
        if (in_array($this->session->userdata('role_id'), $per_role_arr)):
            $data['base_url'] = $this->config->item('base_url');
            $user_id = mysql_real_escape_string($this->input->post('edit_user_id2'));
            $edit_password = mysql_real_escape_string(trim($this->input->post('edit_password')));
         
               $data = array(
                    'password' => base64_encode($edit_password)
