...
<script>
    ...
    function addModal(d_id){ //alert(d_id);  

        $.ajax({
            type: "Post",
            url: "<?php echo site_url('general_request_filter/getData');  ?>",
            data: {'d_id':d_id} ,
			...
			if(data != ''){   
					...
					var is_filter_change=ob[0].is_filter_change; 	 // alert(data); 	 
	...
	function getDateSearch(){
		...
		if(date_from!='' && date_to!=''){
			$.ajax({
				type: "Post",
				url: "<?php echo site_url('general_request_filter/getDateSearch');  ?>",
